...
void TCPMinnowSocket<AdaptT>::_initialize_TCP(const TCPConfig &config) {
  ...
  _eventloop.add_rule(
      ...
      [&] {
        ...
        // debugging output:
        if (_thread_data.eof() and _tcp.value().sender().sequence_numbers_in_flight() == 0 and
            not _fully_acked) {
  ...
  _eventloop.add_rule(
      ...
      [&] {
        return _tcp->inbound_reader().bytes_buffered() or
               ((_tcp->inbound_reader().is_finished() or _tcp->inbound_reader().has_error()) and
...
void TCPMinnowSocket<AdaptT>::connect(const TCPConfig &c_tcp, const FdAdapterConfig &c_ad) {
  ...
  if (_tcp->sender().sequence_numbers_in_flight() != 1) {
    throw runtime_error(
        "After TCPConnection::connect(), expected sequence_numbers_in_flight() == 1");
  }
...
void TCPMinnowSocket<AdaptT>::listen_and_accept(const TCPConfig &c_tcp,
  ...
  _tcp_loop(
      [&] { return (not _tcp->has_ackno()) or (_tcp->sender().sequence_numbers_in_flight()); });
